...
```
Returns are used to end functions. Console.log is used to display the code.
```

**_What's the difference between defining and calling a function?_**

```
Defining a function means creating a function and specifying what it does. Calling a function means getting the function to do what its supoosed to do
```
...
```
a string is anything under quotion marks
```

**_Number_**

```
assining a name/number to a string
```
...
```
is used to list items in a certain order or seprated someway
```
